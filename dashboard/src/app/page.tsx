...
export default async function RosterPage() {
  ...
  return (
    <main className="flex min-h-dvh w-full flex-col">
      ...
      <div className="mx-auto flex w-full max-w-[1400px] flex-col px-6 pb-10">
        {/* Título grande, al estilo de las «large titles» de iOS: la barra de
            navegación lleva la identidad y el cambio de mundo, y el título de
            la pantalla vive aquí abajo, con aire y a un tamaño que se lee desde
            el fondo de la sala. Antes competía con el wordmark en la misma
            línea de 14 px de alto. */}
        <div className="flex items-baseline justify-between gap-4 pb-4 pt-6">
          <div className="flex items-baseline gap-3">
            <h1 className="text-[28px] font-semibold leading-none tracking-[-0.03em]">Schedule</h1>
            {roster.practitioner ? (
              <span className="text-sm text-ink-3">{roster.practitioner}</span>
            ) : null}
          </div>
          <span className="text-2xs text-ink-3">
            {roster.entries.length} patient{roster.entries.length === 1 ? '' : 's'}
          </span>
        </div>

        {roster.entries.length === 0 ? (
          // Una agenda vacía es un estado legítimo —proyecto recién creado—, no
          // un fallo. Se pinta con la salida delante en vez de dejar al médico
          // buscando dónde se empieza.
          <div className="tile flex flex-col items-center gap-3 rounded-tile px-5 py-10 text-center">
            <p className="text-sm text-ink-3">No patients in this Medplum project.</p>
            <NewPatientDialog
    ...
    >
      ...
      <div className="min-w-0">
        <p className="truncate text-xs text-ink-2">
          {entry.activeConditions.length > 0 ? entry.activeConditions.join(' · ') : 'No active problems'}
        </p>
        <p className="truncate text-2xs text-ink-3">
          {entry.activeMedicationCount} active medication{entry.activeMedicationCount === 1 ? '' : 's'}
        </p>
      </div>
      ...
      <div className="min-w-0">
        {headlineMetric ? (
          ...
          <span className="text-2xs text-ink-3">No results</span>
        )}
      </div>
      ...
      <div className="min-w-0 text-right">
        ...
        <p className="truncate text-2xs text-ink-3" title={nextAppointment?.description ?? undefined}>
          {nextAppointment?.description ?? 'No appointment scheduled'}
        </p>
      </div>
